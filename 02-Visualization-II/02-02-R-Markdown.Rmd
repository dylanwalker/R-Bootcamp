...
### From `ggplot2`
We have `r nrow(diamonds)` diamonds in this dataset. Only `r sum(diamonds$carat>2.5)` are bigger than 2.5 carats. 

Notice in the above that we used inline r code by wrapping a code statement with the backticks to run a piece of code and display only its output inline. We used this to determine and display the number of rows in the diamonds dataset, as well as the number of diamonds that have carat > 2.5. 

Here are a few rows of the diamonds dataframe:
...
----
You might be wondering what is happening on the backend when we create the nice markdown visualization from our R markdown file.
