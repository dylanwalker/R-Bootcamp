...
### Aside: What to do when a dataframe doesn't have a PK? 
#### Understand & resolve duplicates 

We could look into why this occurs. It turns out `weather` has another variable called `time_hour` that has greater resolution and there are no duplicates in the combination `(time_hour,origin)`:
```{r}
weather %>% 
  group_by(origin,year,month,day,hour) %>% 
  mutate(n=n()) %>% 
  ungroup() %>% # this gives me back the original, ungrouped data (replicating the n column across all entries in the group)
  filter(n>1) %>% 
  mutate(th_f = as.numeric(time_hour)) %>% # show time_hour as a double
  select(origin,year,month,day,hour,time_hour,n)
```

Hmm, only 6 "duplicated" rows. `th_f` shows that time_hour is actually different at a very fine resolution (so we can't even see it when we display the `time_hour` column.)
...
We could work with the `time_hour` variable, but a simpler approach  is to remove the duplicates using `distinct()` (here, only the first row in each group is kept; another solution could be to average the weather metrics in each group):

```{r}
weather_nodup <- weather %>% distinct(year,month,day,hour,origin, .keep_all=TRUE)
```
