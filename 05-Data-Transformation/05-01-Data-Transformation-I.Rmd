...
* `filter()` picks cases (rows from a dataframe) based on their values.
* `arrange()` changes the ordering of the rows.
* `select()` picks variables (columns from a dataframe) based on their names.
* `mutate()` adds new variables to a dataframe that are functions of existing variables
...
Missing values are always sorted at the end (whether or not you use `desc()`:
...
```{r}
select(flights, time_hour, airtime = air_time, everything())
```

This brings `time_hour` and `airtime` to the beginning of the dataframe and includes all the other columns after it. 